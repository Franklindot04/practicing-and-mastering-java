...
class FailurePlan { private final Deque<FailureType> failures=new ArrayDeque<>(); FailurePlan(FailureType... types){failures.addAll(Arrays.asList(types));} void maybeFail() throws PlannedFailure { FailureType next=failures.isEmpty()?FailureType.NONE:failures.removeFirst(); if(next!=FailureType.NONE) throw new PlannedFailure(next); } }
